describe("Test of kita Gazetteer", () => {
  ...
  it("Tooltip contains a district", () => {
    cy.get(".rbt-input-main").should("be.visible").type("Carnaper Platz");
    cy.get(".rbt-menu").should("be.visible");
    cy.get('.rbt-menu li[aria-label="Carnaper Platz"]').should("be.visible");
  });
  it("Tooltip contains a name", () => {
  ...
  it("After clicking the Clear button, the input field will be empty", () => {
    cy.get("form").within(() => {
      cy.get(".rbt-input-main").type("parkhaus");
